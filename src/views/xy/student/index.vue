<template>
	<div class="student-main">
		<el-card class="card" shadow="hover">
			<div slot="header"><span>搜索</span></div>
			<el-row :gutter="20">
				<el-col :span="6"><el-input v-model="search.name" size="small" placeholder="根据名字搜索" /></el-col>
				<el-col :span="5">
					<el-select v-model="search.classId" size="small" clearable placeholder="根据班级搜索">
						<el-option v-for="item in classList" :key="item.id" :label="item.className" :value="item.id" />
					</el-select>
				</el-col>
				<el-col :span="6"><el-date-picker v-model="search.startTime" size="small" type="date" placeholder="选择日期时间" /></el-col>
			</el-row>
			<el-row :gutter="20">
		...
		<el-card class="card content" shadow="hover">
			<div slot="header"><el-button type="primary" size="small" @click="goAdd">添加</el-button></div>
			<el-table v-loading="loading" element-loading-text="拼命加载中" :data="studentList" style="width: 100%" border>
				...
				<el-table-column prop="cityText" label="地址" width="300" />
				...
				<el-table-column fixed="right" label="操作" width="300">
					<template slot-scope="scope">
						<el-button @click.native="seedetail(scope.row)">查看</el-button>
						<el-button type="primary" @click.native="goEdit(scope.row.id)">编辑</el-button>
						<el-button type="danger" @click.native="del(scope.row.id)">删除</el-button>
					</template>
				</el-table-column>
			</el-table>
